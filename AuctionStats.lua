...
function AuctionStats:CacheAuctions()
  ...
  local src = rawAPI or AuctionStatsDB.storedAuctions
  ...
  for _, info in ipairs(src) do
    ...
    tinsert(self.dbAuctions, {
      ...
      timeLeft    = TimeBands[info.timeLeft or info.duration] or "?",
  ...
  for _,a in ipairs(self.dbAuctions) do
    tinsert(AuctionStatsDB.storedAuctions, {
      itemKey       = { itemID = a.itemID, itemLevel = a.itemLevel },
      quantity      = a.quantity,
      buyoutAmount  = a.rawBuyout,
      timeLeft      = a.timeLeft,
    })
  end
...
function AuctionStats:CreateDetailWindow()
  ...
  f:SetBackdrop{
    bgFile   = "Interface\\DialogFrame\\UI-DialogBox-Background",
    edgeFile = "Interface\\DialogFrame\\UI-DialogFrameBorder",
    tile     = true, tileSize = 32, edgeSize = 32,
    insets   = { left=8, right=8, top=8, bottom=8 },
  }
  f:SetBackdropColor(0, 0, 0, 1)
...
function AuctionStats:DrawDetail()
  ...
  for i,a in ipairs(filtered) do
    ...
    -- сохраняем ссылку и обновляем тултип-код каждый раз
    ln.link = a.link
    ln:SetScript("OnEnter", function(self)
      GameTooltip:SetOwner(self, "ANCHOR_RIGHT")
